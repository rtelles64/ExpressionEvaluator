...
public class LispExpressionEvaluator
{
	...
	// add statements
        ...
        double result;
        Object operand = inputExprStack.pop();
        if (!operand.equals(operand.toString())) {
            throw new LispExpressionEvaluatorException("No operand for evaluation");
        }

        try {
            while (operand.equals(operand.toString())) {
                evaluationStack.push(Double.parseDouble(operand.toString()));
                operand = inputExprStack.pop();
            }
        } catch (EmptyStackException e){
            throw new LispExpressionEvaluatorException("Stack Empty!");
        }

        String operator = operand.toString();
